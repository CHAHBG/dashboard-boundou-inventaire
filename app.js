...
function initializeFilters() {
    ...
    seuilSlider.addEventListener('input', function() {
        seuilValue.textContent = this.value;
        applyFilters();
    });
    ...
    // Problem filters
    document.getElementById('quarantineFilter').addEventListener('change', applyFilters);
    document.getElementById('jointureFilter').addEventListener('change', applyFilters);
...
function applyFilters() {
    const communeSelect = document.getElementById('communeFilter');
    const seuilSlider = document.getElementById('seuilFilter');
    const performanceFilter = document.querySelector('.filter-btn.active').dataset.performance;
    ...
    filteredData = appData.communes_data.filter(commune => {
        ...
        // Seuil filter
        if (commune.parcelles_brutes < seuilMin) {
            return false;
        }
...
function resetFilters() {
    ...
    document.querySelector('.filter-btn[data-performance="all"]').classList.add('active');
...
function updateCommunesChart() {
    const labels = filteredData.map(c => c.commune);
    const brutesData = filteredData.map(c => c.parcelles_brutes);
    const sansDoublonsData = filteredData.map(c => c.parcelles_sans_doublons);
    const postTraiteesData = filteredData.map(c => c.parcelles_post_traitees);
    
    charts.communes.data.labels = labels;
    charts.communes.data.datasets[0].data = brutesData;
    charts.communes.data.datasets[1].data = sansDoublonsData;
    charts.communes.data.datasets[2].data = postTraiteesData;
...
function updatePerformanceChart() {
    const labels = filteredData.map(c => c.commune);
    const performanceData = filteredData.map(c => c.taux_jointure_ok);
...
function updateTable() {
    ...
    paginatedData.forEach(commune => {
        ...
        const tauxJointure = commune.taux_jointure_ok;
        const tauxClass = tauxJointure >= 20 ? 'high' : tauxJointure >= 10 ? 'medium' : 'low';
        ...
        row.innerHTML = `
            ...
            <td><span class="taux-performance ${tauxClass}">${tauxJointure}%</span></td>
        `;
...
function initializeTooltips() {
    ...
    document.querySelectorAll('th[data-tooltip]').forEach(element => {
        element.addEventListener('mouseenter', function(e) {
            const text = this.dataset.tooltip;
            showTooltip(e, text);
        });
...
// Initialize modal for fullscreen charts
...
function openFullscreenChart(chartId) {
    ...
    // Create fullscreen version of chart
    ...
    modal.fullscreenChart = fullscreenChart;
